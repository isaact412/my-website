# Adds the server bible (big-picture server knowledge + character sheets) WITHOUT overwriting your other changes.
...
PATCH_FILE=$(mktemp)
curl -fsSL https://raw.githubusercontent.com/isaact412/my-website/claude/fervent-lamport-jh3erl/discord-ai-bot/patches/bible_upgrade.patch -o "$PATCH_FILE" \
  || { echo "❌ couldn't download the patch. nothing changed."; exit 1; }
if [ -f bot/memory/bible.py ]; then
  echo "ℹ️  already installed (bot/memory/bible.py exists). nothing changed."; exit 0
fi
if patch -p1 -N --dry-run < "$PATCH_FILE" > /tmp/bible_upgrade_check.txt 2>&1; then
  patch -p1 -N < "$PATCH_FILE" > /dev/null
  if .venv/bin/python -m py_compile bot/memory/bible.py bot/ai/prompts.py bot/services/responder.py; then
    echo "✅ server bible installed. your other changes were kept."
  ...
  cat /tmp/bible_upgrade_check.txt
fi
