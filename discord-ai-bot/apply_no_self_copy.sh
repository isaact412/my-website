# Stops the bot from copying its own old replies (it copies the server instead).
...
if grep -q "drop_own_old_lines" bot/services/responder.py 2>/dev/null; then
...
PATCH_FILE=$(mktemp)
curl -fsSL https://raw.githubusercontent.com/isaact412/my-website/claude/fervent-lamport-jh3erl/discord-ai-bot/patches/no_self_copy.patch -o "$PATCH_FILE" \
  || { echo "❌ couldn't download the patch. nothing changed."; exit 1; }
if patch -p1 -N --dry-run < "$PATCH_FILE" > /tmp/no_self_copy_check.txt 2>&1; then
  patch -p1 -N < "$PATCH_FILE" > /dev/null
  if .venv/bin/python -m py_compile bot/services/responder.py; then
    echo "✅ self-copy fix installed. your other changes were kept."
  ...
  cat /tmp/no_self_copy_check.txt
fi
